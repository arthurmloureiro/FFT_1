...
#pl.yscale("log")
pl.grid(1)
pl.ylabel(r"$\langle \hat{P}_c / P_c \rangle$")
pl.xlabel("$k hMpc^{-1}$")
pl.title(r"$\hat{P}_{c}^{P+G}(k)$")
for i in range(realiz):
	pl.plot(k_bar[1:], ((PN_gauss[:,i]-(dV/n_bar + 50) )/b_gauss)/Pk(k_bar[1:]), "o")
	#pl.plot(k_bar[1:], ((PN_gauss[:,i]-(dV/n_bar + 50) )/b_gauss), "o")
	#pl.plot(k_bar[1:], PN_gauss[:,i], "o")
#pl.plot(k_r,P_k, label="CAMB")
...
pl.ylabel(r"$\langle \hat{P}_c / P_c \rangle$")
pl.xlabel("$k hMpc^{-1}$")
pl.title("$\hat{P}_{c}^{P}(k)$")
for i in range(realiz):
	pl.plot(k_bar[1:], ((PN_poiss[:,i]-(dV/n_bar + 50) )/b_poiss)/Pk(k_bar[1:]), "o")
	#pl.plot(k_bar[1:], ((PN_poiss[:,i]-(dV/n_bar + 50) )/b_poiss), "o")
	#pl.plot(k_bar[1:], PN_poiss[:,i], "o")
#pl.plot(k_r,P_k, label="CAMB")
#legend = pl.legend(loc=0, shadow=True)
#frame = legend.get_frame()
#frame.set_facecolor('0.90')
...
def env(k_):
	fish = (n_bar*np.mean(bias_esti_gauss1)*Pk(k_))/(1. + n_bar*np.mean(bias_esti_gauss1)*Pk(k_))
	return (4.*(np.pi**2.))*np.power((dV*(k_**2.)*(np.max(k.matrix)/n_bins)),-1.)*np.power(fish, -2.)
c1=0.025
pl.figure()
pl.title("Errors")
pl.subplot(211)
...
pl.xlabel("$k$")
pl.plot(k_bar[1:], env(k_bar[1:])/Pk(k_bar[1:])-c1, label="$(\sigma (P)/P)^2$")
pl.errorbar(k_bar[1:],(P_bla_gauss-P_bla_gauss), yerr=err_gauss, label="Gauss + Poiss")
pl.errorbar(k_bar[1:],(P_bla_poiss-P_bla_poiss), yerr=err_poiss, color="k", label="Poiss")
legend = pl.legend(loc=0, shadow=True)
frame = legend.get_frame()
frame.set_facecolor('0.90')
pl.subplot(212)
pl.title("Gaussian Error")
pl.plot(k_bar[1:], env(k_bar[1:])/Pk(k_bar[1:])-c1, label="$(\sigma (P)/P)^2$")
pl.errorbar(k_bar[1:],(P_bla_gauss-P_bla_gauss), yerr=(err_gauss - err_poiss), color="r", label="Gaussian Only")
pl.ylabel("$<(P_g(k)-SN)/b^2> - P_{camb}(k)/P_{camb}(k)$")
pl.xlabel("$k$")
legend = pl.legend(loc=0, shadow=True)
frame = legend.get_frame()
...
pl.figure(4)
pl.title("Histogram for different $k$ in $P_{gg}^{P+G}(k)$")
pl.subplot(221)
pl.title("$k=1 k_0$")
pl.hist(PN_gauss[1,:], bins=30)
pl.subplot(222)
pl.title("$k=15 k_0$")
pl.hist(PN_gauss[15,:], bins=30)
pl.subplot(223)
pl.title("$k=35 k_0$")
pl.hist(PN_gauss[35,:], bins=30)
pl.subplot(224)
pl.title("$k=47 k_0$")
pl.hist(PN_gauss[47,:], bins=30)

pl.figure()
pl.title("Histogram for different $k$ in $P_{gg}^{P}(k)$")
pl.subplot(221)
pl.title("$k=1 k_0$")
pl.hist(PN_poiss[1,:], bins=30, color="g")
pl.subplot(222)
pl.title("$k=15 k_0$")
pl.hist(PN_poiss[15,:], bins=30, color="g")
pl.subplot(223)
pl.title("$k=35 k_0$")
pl.hist(PN_poiss[35,:], bins=30, color="g")
pl.subplot(224)
pl.title("$k=47 k_0$")
pl.hist(PN_poiss[47,:], bins=30, color="g")
